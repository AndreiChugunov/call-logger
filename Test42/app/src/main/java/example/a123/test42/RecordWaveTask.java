...
class RecordWaveTask extends AsyncTask<File, Void, Object[]> {
        ...
        for (int rate : new int[] { 8000, 11025, 22050, 44100 }) {
            for (short audioFormat : new short[] { AudioFormat.ENCODING_PCM_8BIT, AudioFormat.ENCODING_PCM_16BIT }) {
                for (short channelConfig : new short[] { AudioFormat.CHANNEL_IN_MONO, AudioFormat.CHANNEL_IN_STEREO }) {
                    try {
                        Log.d("MyTag", "Attempting rate " + rate + "Hz, bits: " + audioFormat + ", channel: "
                                + channelConfig);
                        ...
                        Log.e("MyTag", rate + "Exception, keep trying.",e);
                    }
                }
            }
        }
    ...
    protected void onPostExecute(Object[] results) {
        ...
        if (ctx != null) {
            if (throwable == null) {
                ...
                Log.d("MyTag",String.format(Locale.getDefault(), "%.2f MB / %d seconds",
                        size, time));
            } else {
                // Error
                Toast.makeText(ctx, throwable.getLocalizedMessage(), Toast.LENGTH_LONG).show();
                Log.d("MyTag", throwable.getLocalizedMessage());
            }
        }
        else  {
            double size = (long) results[0] / 1000000.00;
